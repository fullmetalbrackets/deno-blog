...
Secure Copy is a command line utility that transfers files between the local
host and a remote host via the SSH protocol. You will be prompted for the remote
user's password, or you can use an
<a href="https://arieldiaz.codes/blog/generating-an-ssh-key-pair" target="_blank">authorized
SSH key</a>.
...
- `-r`: Recursively copy directories (copy directory contents and
  sub-directories and their content)
- `-P`: Specifies an SSH port on destination host, use if destination uses SSH
  port other than 22
...
To copy from the local host rather than to it, put the remote host's info first
and the local host info after:
...
With **SCP** its even possible to transfer between two other systems, as long as
your local machine has access to both via SSH or password.
